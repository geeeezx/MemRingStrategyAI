...
interface FileUploadProps {
  ...
  disabled?: boolean;
  className?: string;
  compact?: boolean; // For smaller spaces like follow-up nodes
}
...
const FileUpload: React.FC<FileUploadProps> = ({ onFileSelect, disabled = false, className = '', compact = false }) => {
        ...
        {selectedFile ? (
          ...
          <div className={compact ? "space-y-2" : "space-y-4"}>
            <div className="flex items-center justify-center space-x-3">
              <span className={compact ? "text-xl" : "text-2xl"}>{selectedFile.icon}</span>
              <div className="text-left">
                <p className={`font-medium ${compact ? 'text-sm' : ''} ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>
          ...
          // Upload Prompt
          <div className={compact ? "space-y-2" : "space-y-4"}>
            <div className={`mx-auto flex items-center justify-center rounded-full bg-blue-100 dark:bg-blue-900/30 ${
              compact ? 'w-8 h-8' : 'w-12 h-12'
            }`}>
              <svg className={`text-blue-600 dark:text-blue-400 ${compact ? 'w-4 h-4' : 'w-6 h-6'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
              </svg>
            </div>
            
            <div>
              <p className={`font-medium ${compact ? 'text-sm' : 'text-lg'} ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>
                Upload a file
              </p>
              <p className={`text-xs mt-1 ${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'}`}>
                {compact ? 'Drag & drop or click' : 'Drag and drop or click to browse'}
              </p>
            </div>
